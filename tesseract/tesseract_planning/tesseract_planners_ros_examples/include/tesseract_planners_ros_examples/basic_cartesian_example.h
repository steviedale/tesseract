...
#include <tesseract_planners_ros_examples/example.h>
...
{
public:
  BasicCartesianExample(int steps, std::string method, std::string urdf_xml_string, std::string srdf_xml_string, std::string trajopt_config)
    : Example(urdf_xml_string, srdf_xml_string), steps_(steps), method_(std::move(method)), trajopt_config_(trajopt_config)
  {
  }
